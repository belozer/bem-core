...
function initEntity(entityName, domElem, params, ignoreLazyInit, callback) {
    ...
    if(entityCls.__bemEntity) {
        ...
        needInit = !entityCls._liveInitable || ignoreLazyInit || params.live === false;
    }
...
var BemDomEntity = inherit(/** @lends BemDomEntity.prototype */{
    ...
    _deprecated : function(method, needUse) {
        var entityName = this.__self.getEntityName();
        window.console.warn('Method \'' + method + '\' for \'' + entityName + '\' deprecated! Your need use ' + needUse);
    },

    on : function() {
        this._deprecated('on', '_events');
        this._events().on.apply(this._events(), arguments);
        return this;
    },

    once : function() {
        this._deprecated('once', '_events');
        this._events().once.apply(this._events(), arguments);
        return this;
    },

    un : function() {
        this._deprecated('un', '_events');
...
$.fn.bem = function(BemDomEntity, params) {
    var entityName = BemDomEntity;

    if(functions.isFunction(BemDomEntity)) {
        entityName = BemDomEntity.getEntityName();
    }

    var entity = initEntity(entityName, this, params, true);
...
bemDom = /** @exports */{
    _uniqIdToDomElems : uniqIdToDomElems,
    _uniqIdToEntity : uniqIdToEntity,
    _domElemToParams : domElemToParams,
    _initEntity : initEntity,
    _getParentDomNode : function(ctxId) {
        return domNodesToParents[ctxId];
    },
